...
ad_proc -public qal_demo_contact_create {
    ...
    set co_list [list  \
                     ...
                     time_start "" \
                     time_end "" \
                     url "http://[ad_generate_random_string [randomRange 20]].[ad_generate_random_string [randomRange 20]]/[join [qal_namelur 1]]" \
                     user_id $user_id \
                     created "" \
                     created_by $user_id \
                     trashed_p "0" \
...
ad_proc -public qal_demo_address_write {
    ...
    if { $record_type eq "" } {
        ...
        ns_log Notice "qal_demo_address_write.540: record_type '${record_type}'"
    }
    ...
    if { $is_postal_p } {
    ns_log Notice "qal_demo_address_write.550. contact_id '${contact_id}' instance_id '${instance_id}' array get a_arr '[array get a_arr]'"
        qf_array_to_vars a_arr [qal_address_keys]
    ns_log Notice "qal_demo_address_write.560. contact_id '${contact_id}' instance_id '${instance_id}' array get a_arr '[array get a_arr]'"
        set word_count [randomRange 3]
        incr word_count
        set syllables [randomRange 3]
        incr syllables
        set address0 [join [qal_namelur 3 3 ]]
        append address0 ", ref "
        ...
        append email [ad_generate_random_string [randomRange 10]] "@" [join [qal_namelur 1 7]]
        set email [string tolower $email]
        set cc [join [qal_namelur 1 7 "_"]]
        append cc "@" [join [qal_namelur 1 7] ]
        set cc [string tolower $cc]
        append cc ", \"" [join [qal_namelur]] "\" <" 
        append cc [string tolower [qal_namelur 1 7 "_"]] "@" [string tolower [qal_namelur 1 7]] ">"
        set bcc "admin@[ad_url]"
        set notes "a postal address generated by qal_demo_address_write"
        ns_log Notice "qal_demo_address_write.570. contact_id '${contact_id}' instance_id '${instance_id}' array get a_arr '[array get a_arr]'"
        qf_vars_to_array [qal_address_keys] a_arr
        ns_log Notice "qal_demo_address_write.580. contact_id '${contact_id}' instance_id '${instance_id}' array get a_arr '[array get a_arr]'"

    } else {
        set account_name [string tolower [join [qal_namelur 1 7 "_"]]]
        set notes "a non-postal address generated by qal_demo_address_write"
    }

    qf_vars_to_array [qal_other_address_map_keys] a_arr
    ns_log Notice "qal_demo_address_write.589. contact_id '${contact_id}' instance_id '${instance_id}' array get a_arr '[array get a_arr]'"
